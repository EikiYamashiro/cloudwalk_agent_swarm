...
def _resolve_user_identity() -> tuple[str, str]:
    context_user_id, context_username = get_current_user()

    resolved_user_id = (context_user_id or "").strip()
    resolved_username = (context_username or "").strip()
...
def send_slack_message(message: str) -> dict:
    ...
    """
    resolved_user_id, resolved_username = _resolve_user_identity()
